...
Users can transfer data between Gaea and Zeus' High Performance Storage System (HPSS) through the use of the ``hsi`` and ``htar`` commands. These commands are only available on Gaea's Remote Data Transfer Nodes (RDTNs). See :ref:`modules <data-and-transfers>` for more information.
...
F2 is a 33PB Lustre Filesystem. Certain limitations apply to F2. For instance, performance will start to degrade after utilization exceeds 80% on a file system. Therefore, using well-informed I/O, managing the quota, and using lustre storage tools (when searching your files or managing your space) is important. 
...
Generally, users submit jobs by writing a batch script and submitting the job to Slurm with the ``sbatch`` command. The ``sbatch`` command takes a number of options. The options you are allowed to specify are the set of options used for the SLURM batch system. For a list of options, use ``man sbatch`` page. 
